import { Class, Interface, Method, Namespace, Type } from "./types"
...
const createProperties = (c: Class, ns: Namespace) => c.properties.length === 0 ? '' : (
    filterDupliciteStaticProperties(c, ns)
        .map(p => propertyAccessMap[p.access] + p.name + ';\n')
        .join('\n') + '\n'
);
...
const createComment = (o: Method | Class | Interface) => {
    ...
    if ('arguments' in o) {
        o.arguments.forEach(a => {
            ...
            commentSections.push('@param ' + (a.type ? '{' + (typeof a.type === 'string' ? a.type : a.type.join(' | ')) + '} ' : '') + createName() + ' ' + a.comment);
        })
    }
